export default class TestTimer {
	...
	schedule(func, delay) {
		...
		this.log('set timer', id, 'to run after', delay, 'ms')
	...
	async _trigger(timer) {
		return await new Promise((resolve) => {
			this.log('fast forward', timer.at - this._now, 'ms', 'to trigger timer', timer.id)
			this._now = timer.at
			this._onSchedule = () => {
	...
	async next() {
		this.log('trigger closest timer')
	...
	async fastForwardToLast() {
		this.log('fast forward to last scheduled timer')
	...
	async fastForward(timeAmount) {
		...
		if (this.timers.length === 0) {
			this.log('fast forward', timeAmount, 'ms')
		...
		// via something like `setTimeout(..., 0)` would "unstuck" it, but then
